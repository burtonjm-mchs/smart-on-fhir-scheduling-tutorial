...
function slotSearch() {
  ...
  slotParams["start"] = {
    $ge: form.elements["date-start"].value,
    $lt: form.elements["date-end"].value,
  };
      ...
      function (slots) {
        ...
        if (slots.length) {
          ...
          slots.forEach(function (slot) {
            slotsHTML =
              slotsHTML +
              slotHTML(slot.id, slot.type.text, slot.start, slot.end);
          });
        ...
        // If no Slots matched the criteria, inform the user
          ...
          renderSlots(
            "<p>No Slots found for the selected query parameters.</p>"
          );
        }
      },
...
function renderAppointment(appointmentLocation) {
  clearUI();
  $("#appointment").html(
    "<p>Created Appointment " + appointmentLocation.match(/\d+$/)[0] + "</p>"
  );
  ...
  FHIR.oauth2.ready(function (smart) {
    smart.api.fetchAll({ type: "Patient", query: patientParams }).then(
      ...
      function (patients) {
        ...
        if (patients.length) {
          ...
          patients.forEach(function (patient) {
            ...
            patientsHTML =
              patientsHTML +
              patientHTML(slotReference, patient.id, patientName);
          });
        ...
        // If no Patients matched the criteria, inform the user
          ...
          renderPatients(
            "<p>No Patients found for the selected query parameters.</p>"
          );
        }
      },
      ...
      function () {
        clearPatientUI();
        $("#patient-errors").html(
          "<p>Failed to read Patients from FHIR server</p>"
        );
  ...
  FHIR.oauth2.ready(function (smart) {
    smart.api.create({ resource: patientBody }).then(
      ...
      function (patient) {
        ...
        alert(
          "Created Patient " +
            patient.headers("Location").match(/\d+$/)[0] +
